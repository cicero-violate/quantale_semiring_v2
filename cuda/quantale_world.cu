    ...
    for (int g = 0; g < group_len; ++g) {
        ...
        if (target_hop >= 0 && target_hop < N) {
            for (int idx = 0; idx < MATRIX_LEN; ++idx) {
                ...
                int hop = witness[cidx];
                if (hop != target_hop || consumed[src * N + hop] != 0) {
                    continue;
                }
...
extern "C" __global__ void tensor_quantale_commit_batch(
    ...
    if (tid == 0) {
        ...
        for (int i = 0; i < decision_count; ++i) {
            ...
            if (decisions[i].blocked == 0 && decisions[i].halted == 0 && src >= 0 && src < N && hop >= 0 && hop < N) {
                consumed[src * N + hop] = 1;
                next_active[hop] = 1;
                committed += 1;
            }
...
__device__ __forceinline__ float exploration_token_value(
    ...
    float receipt_weight,
    float entropy_penalty
) {
    float depth = (float)(token.depth + 1);
    return (token.confidence / depth) - (token.cost / depth)
        + (token.safety / depth)
...
__device__ int exploration_first_hop(const ExplorationToken* tokens, int token_id) {
    ...
    int parent = tokens[current].parent;
    while (parent >= 0) {
...
extern "C" __global__ void tensor_quantale_seed_exploration(
    ...
    for (int i = 0; i < max_tokens; ++i) {
        tokens[i].strategy_id = -1;
        tokens[i].node = -1;
        tokens[i].depth = -1;
        tokens[i].confidence = 0.0f;
        tokens[i].cost = COST_INFINITY;
        tokens[i].safety = 0.0f;
        tokens[i].novelty = 0.0f;
        tokens[i].receipt_prior = 0.0f;
        tokens[i].entropy = 0.0f;
        tokens[i].parent = -1;
    ...
    for (int sid = 0; sid < strategy_count && token_count[0] < max_tokens; ++sid) {
        ...
        if (confidence <= BOTTOM && safety <= BOTTOM) {
            continue;
        }
...
extern "C" __global__ void tensor_quantale_expand_tokens(
    ...
    for (int parent_id = 0; parent_id < original_count && token_count[0] < max_tokens; ++parent_id) {
        ...
        if (parent.depth != source_depth || parent.depth >= max_depth) {
            continue;
        }
        int src = parent.node;
        if (src < 0 || src >= N) {
...
extern "C" __global__ void tensor_quantale_select_topk_tokens(
    ...
    for (int slot = 0; slot < limit; ++slot) {
        ...
        for (int i = 0; i < count; ++i) {
            ...
            if (terminal_visits[terminal] >= max_terminal_visits || first_hop_visits[hop] >= max_first_hop_visits) {
                continue;
            }
            bool already = false;
            ...
            float adjusted = scores[i] - repeat_penalty * (float)(terminal_visits[terminal] + first_hop_visits[hop]);
            if (adjusted > best) {
                best = adjusted;
                best_id = i;
                best_hop = hop;
