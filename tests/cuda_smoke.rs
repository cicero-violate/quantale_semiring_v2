...
mod cuda_smoke {
    ...
    #[test]
    fn control_flow_par_effects_independent() -> Result<(), Box<dyn std::error::Error>> {
        use quantale_semiring_v2::EffectTable;
        let mut world = match TensorQuantaleWorld::empty() {
            Ok(w) => w,
            Err(e) => {
                eprintln!("skip: {e}");
        ...
        world.load_control_table(vec![], effects)?;

        assert!(
            world.check_effects_independent(0, 1)?,
            "non-overlapping effects should be independent"
        );
        Ok(())
    }

    /// Phase-4: two effect-table entries where one writes what the other reads
    /// are reported as NOT par-eligible.
    #[test]
    fn control_flow_par_effects_conflict() -> Result<(), Box<dyn std::error::Error>> {
        use quantale_semiring_v2::EffectTable;
        let mut world = match TensorQuantaleWorld::empty() {
            Ok(w) => w,
            Err(e) => {
